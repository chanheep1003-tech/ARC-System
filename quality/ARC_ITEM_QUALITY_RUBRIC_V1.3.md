...
- 사상가·이론 비판 문항이 상대의 실제 입장 또는 정당한 함의가 아니라 허수아비를 공격함
- '그 자체로/항상/오직/어떤 경우에도' 같은 절대 표현이 자료가 뒷받침하는 강도보다 과도하여 정오판단이 표현 해석에 좌우됨
...
QUALITY_RUBRIC_VERSION: ARC-IQR-V1.3

## 10. EXPLICIT-CUE / RESTATEMENT CEILING
다음 중 하나면 고난도·PREMIUM으로 인정하지 않는다.
- <보기>나 지문이 판단 원리를 직접 말해 주고 정답이 그 문장을 거의 그대로 재진술함
- 정답이 제시문 한 문장을 단순 부정/반전한 것만으로 결정됨
- 실제 사고과정이 '문장 찾기 → 같은 말/반대말 고르기' 1단계임

적용:
- REASONING_QUALITY <= 9/15
- DISTRACTOR_QUALITY <= 13/20
- TOTAL <= 86
- D4/D5 금지
- BANK_A 금지
- FLAG = EXPLICIT_CUE_RESTATEMENT

단, 제시 원리를 서로 다른 두 자료/사례에 적용하거나 충돌 조건을 함께 판정해야 하면 이 상한을 자동 적용하지 않는다.

## 11. DISTRACTOR DISTANCE AUDIT
오답은 정답과 같은 판단축에서 경쟁해야 한다.

다음은 감점:
- 작품/자료와 전혀 무관한 결론
- 과도한 극단 표현만으로 즉시 탈락
- 출제 범위에서 한 번도 다루지 않은 방향으로 과장
- 정답과 비교할 필요 없이 문장 자체가 부자연스러워 제거 가능

5지선다에서 2개 이상이 위 조건이면:
- DISTRACTOR_QUALITY <= 13/20
- TOTAL <= 87
- BANK_A 금지
- FLAG = DISTRACTOR_DISTANCE

3개 이상이 즉시 제거 가능하면 기존 WEAK_DISTRACTOR_CEILING을 적용한다.

## 12. BLIND SCORE RECHECK
Generator의 최초 점수는 PROVISIONAL이다.

다음 문항은 최초 점수/등급을 보지 않은 상태로 독립 재채점한다.
- 모든 BANK_A 후보
- 모든 D4/D5 후보
- 세트 sentinel
...
- 총점 차이 >= 5점
- D4/D5 → D3 이하
- BANK_A → BANK_B/REVISE
중 하나면 더 낮은 판정을 우선하고 이유를 기록한다.

## 13. ZERO-REJECTION / PREMIUM-RATE AUDIT
20문항 이상 세트에서 다음은 자동 PASS가 아니라 '과대평가 의심'이다.
- REVISE/DISCARD가 0개
- BANK_A 비율 > 30%
- D4+D5 비율 > 45%

이 경우 최소 6문항을 추가 blind recheck한다.
구성:
- 최고점 2
- 중간점 2
- 최저 PASS 2

추가 검수 완료 전 세트 전체를 BANK 승격하지 않는다.

## 14. PREMIUM DISTRACTOR REQUIREMENT
BANK_A/PREMIUM은 최소 2개의 오답이 첫 독해에서 실제 후보로 남아야 한다.
각 오답마다:
- 어떤 오개념/조건 누락/관찰-추론 혼동 때문에 선택 가능한지
- 어떤 근거 하나 때문에 최종 탈락하는지
를 한 문장씩 기록한다.

'정답이 맞고 나머지는 틀림' 수준의 근거는 PREMIUM 근거가 아니다.

## 15. CLAIM ATTRIBUTION / PHILOSOPHY AUDIT
사상가·정치철학·윤리·법사상 문항은 다음을 별도 검수한다.
...
PASS 조건:
- 정답이 EXPLICIT_POSITION 또는 범위 내 전제로 충분히 도출되는 SUPPORTED_IMPLICATION에 근거
- 비판 문항은 상대의 실제 전제/함의를 직접 겨냥
- 출제자의 극단적 귀결을 대상 사상가의 실제 승인처럼 표현하지 않음
- 절대 표현의 강도가 source wording과 일치

STRAWMAN_RISK가 정답 경로에 포함되면 HARD QUALITY FAIL.
AMBIGUOUS_ABSOLUTE가 핵심 판단을 좌우하면 REVISE이며, 수정 전 BANK_PASS 금지.
단순 사상가 이름-키워드 매칭만 요구하면 DIRECT_RECALL_CEILING을 적용한다.

## 16. OPTION-SHUFFLE INTEGRITY
정답 위치를 편집하기 위해 선지를 재배열한 문항은 새 문항으로 간주해 UNIQUE_ANSWER를 다시 검사한다.
필수:
- 정답 '내용' 불변
- 오답 의미 불변
- 번호만 임의 치환 금지
- ANSWER_KEY 실제 위치 동기화
- PASS A/B 재실행
- 조합형/표형/OX형은 재배열 후 대응관계 재검산

번호 분포 개선 후 정답 내용이나 논리관계가 바뀌면 FAIL.

END ARC ITEM QUALITY RUBRIC V1.3
