...
## 10. PHYSICAL APPROVAL EVIDENCE GATE
FINAL_RELEASED requires evidence that the user's approval refers specifically to the physical print review, not merely to the digital result or workflow success.

Do NOT treat generic positive reactions as physical approval by themselves:
- 좋아
- 좋다
- 좋다니까
- 성공적이야
...
These may acknowledge the PDF, storage, workflow, or conversation rather than a printed copy.

PHYSICAL_HUMAN_APPROVAL=PASS is allowed only when one of the following is true:

A. Explicit physical-review statement:
- 출력해봤는데 문제 없어
- 인쇄본 확인했고 괜찮아
- 실물로 봤고 통과
- 프린트 확인 완료
- equivalent wording that clearly identifies a physical/printed copy

B. Direct answer to an immediately preceding explicit physical-review confirmation prompt where the accepted token is defined in advance.

A bare 네 / 응 / 좋아 / PASS is insufficient unless the preceding prompt explicitly defines that exact token as confirmation of physical print inspection.

Required fields before FINAL_RELEASED:
PHYSICAL_PRINT_REVIEW = PASS
PHYSICAL_REVIEW_EVIDENCE = <user statement>
PHYSICAL_REVIEW_EVIDENCE_TYPE = EXPLICIT_PRINT_STATEMENT | DEFINED_CONFIRMATION_TOKEN

If evidence is ambiguous:
PHYSICAL_PRINT_REVIEW = PENDING
RELEASE_STATUS = READY_FOR_PHYSICAL_REVIEW
FINAL_RELEASED = false

Do not repeatedly pressure the user for approval. State the current release status once and wait until they actually report print inspection.

## 11. DIGITAL PRAISE VS PHYSICAL APPROVAL
DIGITAL_HUMAN_REVIEW and PHYSICAL_PRINT_REVIEW are separate states.

Praise of:
- PDF appearance on screen
- successful upload
- successful storage verification
- overall production workflow

may update DIGITAL_HUMAN_REVIEW but must never auto-promote PHYSICAL_PRINT_REVIEW.

END ARC HUMAN REVIEW GATE V1.2