# ARC N제 SYSTEM — COMMON GENERATION ENGINE V4.0
# VERSION_DATE: 2026-09-17
# MODE: CONTENT GENERATION ONLY
# PIPELINE: SUBJECT MASTER → CONTENT BUNDLE → ARC PDF 조판 MASTER V2.0
...
- 정답 번호는 억지 균등배분 금지. 단, 한 번호에 비정상적으로 몰리지 않게 확인.
...
PDF_MASTER = ARC_PDF_조판_MASTER_V2.0
...
편집·검증용 정답 데이터.
학생 PDF에 자동 삽입한다는 의미가 아니다.
ARC_N: 학생 PDF에는 넣지 않음.
ARC_FINAL: 기본적으로 학생 시험지에는 넣지 않으며, 필요 시 별도 정답 파일 생성에 사용.
