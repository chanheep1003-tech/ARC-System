# ARC DRIVE WRITE ADAPTER V1.2
VERSION: 1.2
DATE: 2026-09-18
STATUS: ACTIVE-DEV
ROLE: connection-aware ARC persistence for native and binary artifacts
...
## 13. FILE-REFERENCE-FIRST BINARY UPLOAD
For PDF and other binary artifacts, use the cheapest safe transport in this order:
1. connector/runtime file reference (file_uri or equivalent)
2. provider-supported direct binary upload that does not inline the bytes into model context
3. MANUAL_UPLOAD_REQUIRED

INLINE_BASE64_BINARY_UPLOAD=DISABLED_BY_DEFAULT.

Do not convert a normal ARC PDF into a huge inline base64 string merely because a host exposes such an endpoint.
Do not ask the user to choose between wasting tokens and manual upload.

If only inline base64 is available:
- preserve the final file
- record TARGET_FOLDER_ID and intended filename
- set STORAGE_STATUS=MANUAL_UPLOAD_REQUIRED
- continue to READY_FOR_STORAGE_HANDOFF
- perform exact parent verification automatically when the uploaded file becomes visible

## 14. REGISTERED TARGET RESOLUTION
Before asking the user for a destination folder:
1. read ops/ARC_STORAGE_TARGETS_V1.0.yaml
2. check SYSTEM_MANIFEST
3. check locked bundle/handoff metadata

If a matching target ID is already registered, use it without confirmation.

## 15. DIRECT PDF UPLOAD
When the host supports file-reference upload to a parent folder:
- upload directly to the registered parent
- capture returned FILE_ID
- immediately re-read parent_ids for that exact FILE_ID
- set STORAGE_PLACEMENT=PASS only after the exact target is confirmed

This path is preferred over root staging for interactive final PDFs.

END ARC DRIVE WRITE ADAPTER V1.2
