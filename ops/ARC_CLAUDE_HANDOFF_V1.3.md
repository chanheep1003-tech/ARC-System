...
## 15. PHYSICAL REVIEW LANGUAGE GUARD
Claude must distinguish digital approval, storage acknowledgement, and physical print approval.

Generic positive reactions alone do not establish PHYSICAL_PRINT_REVIEW=PASS.
Examples include 좋아, 좋다니까, 성공적이야, 괜찮아, 완벽해, 오케이.

FINAL_RELEASED may be set only when the active HUMAN_REVIEW gate has explicit evidence that a printed or physical copy was inspected.

If the user gives generic praise while RELEASE_STATUS=READY_FOR_PHYSICAL_REVIEW:
- keep PHYSICAL_PRINT_REVIEW=PENDING
- keep FINAL_RELEASED=false
- do not repeat the same approval question

Canonical successful storage output is STORAGE_PLACEMENT=VERIFIED.
Legacy PASS may be read as equivalent input but should not be emitted as the canonical new state.

END ARC CLAUDE HANDOFF V1.3
