...
- only then STORAGE_PLACEMENT=VERIFIED
...
## 7. PHYSICAL REVIEW EVIDENCE
Publisher may set FINAL_RELEASED=true only when the active HUMAN_REVIEW gate provides:
PHYSICAL_PRINT_REVIEW=PASS
and
PHYSICAL_REVIEW_EVIDENCE_TYPE is present.

Generic satisfaction with the digital PDF, upload, storage, or workflow is not sufficient.

If the user says only a generic positive phrase after READY_FOR_PHYSICAL_REVIEW:
- keep HUMAN_REVIEW_STATUS=PENDING_PHYSICAL
- keep RELEASE_STATUS=READY_FOR_PHYSICAL_REVIEW
- keep FINAL_RELEASED=false

Publisher must never manufacture APPROVAL_TYPE=EXPLICIT_USER_APPROVAL from an ambiguous reaction.

## 8. STORAGE STATUS NORMALIZATION
Canonical value:
STORAGE_PLACEMENT=VERIFIED

Legacy STORAGE_PLACEMENT=PASS may be read as equivalent input, but Publisher outputs VERIFIED only.

END ARC PUBLISHER CONTRACT V1.1
