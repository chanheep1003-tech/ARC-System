...
- JIT skill: manifest-registered `arc-typeset-qa` for product isolation, semantic pagination, typography and render QA
...
## 4-A. PRINT LEGIBILITY + REFLOW GATE
Before PDF_QC_STATUS=PASS:

FONT_WEIGHT_AUDIT=PASS
PRINT_LEGIBILITY=PASS
PAGE_OCCUPANCY_AUDIT=PASS
RAW_MARKUP_LEAK=0
STUDENT_METADATA_LEAK=0

ARC_CORE specific:
- CORE main reading flow is hard-locked to ONE_COLUMN
- ARC N° / ARC FINAL two-column selectors or markup may not be reused for CORE body flow
- CORE_TWO_COLUMN_FLOW=0 is required
- Part → Chapter → Section → Functional label hierarchy must remain visually distinct
- headings must keep at least ~3 lines of following body when feasible
- tables split only at row boundaries; figures/diagrams may not be clipped across page boundaries
- fixed-height body page + overflow:hidden is forbidden
- when editable HTML source is available, run a pre-export scrollHeight/clientHeight overflow audit; >2px is HOLD
- no forced cover-verso blank page
- unregistered near-empty page (<25% effective content) = FAIL
- sparse page (<45%) = reflow required unless an explicit editorial exception is documented
- 1–3 line carry-over/orphan is reflow priority
...
END ARC TYPESETTER CONTRACT V1.6
